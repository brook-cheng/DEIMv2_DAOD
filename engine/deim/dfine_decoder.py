...
class MSDeformableAttention(nn.Module):
    ...
    def forward(
        ...
        if reference_points.shape[-1] == 2:
            ...
            # 5D OBB 分支的 θ 通道按 shifted 编码解码为物理角。
            ...
            raise ValueError(
                "Last dim of reference_points must be 2 , 4 or 5, but get {} instead.".format(
...
class LQE(nn.Module):
    def __init__(self, k, hidden_dim, num_layers, reg_max, act="relu", num_reg_dist=4):
        ...
        self.num_reg_dist = num_reg_dist
        self.reg_conf = MLP(
            self.num_reg_dist * (k + 1), hidden_dim, 1, num_layers, act=act
        )
    ...
    def forward(self, scores, pred_corners):
        ...
        prob = F.softmax(
            pred_corners.reshape(B, L, self.num_reg_dist, self.reg_max + 1), dim=-1
        )
