...
仓库所有文档统一置于 `docs/superpowers/`（历史曾设 `openspec/` 双轨，已合并）。按制品类型分目录：

| 系统 | 角色 | 路径 | 内容 |
|------|------|------|------|
| **Superpowers** | 工作区 + 决策 | `docs/superpowers/` | agent 工作流制品与正式决策：探索、设计、实施计划、评审、实验结果、变更提案、能力规范、深度分析 |

正式决策（提案、架构设计、任务分解、能力规范、深度分析）与过程产物（脑暴、计划、评审、实验）统置于 `docs/superpowers/`，按 §3.3 子目录归类。
...
> 正式的变更提案、架构设计、任务分解、能力规范、深度分析等，原置于 `openspec/`，现已合并至上述 superpowers 子目录（提案/设计/规范 → `specs/`，任务 → `plans/`，分析 → `review/`），按文件名日期与主题归类。
...
所有文档归 `docs/superpowers/`，按制品类型选子目录：

- **探索 / 脑暴** → `design/`
- **实施计划 / 任务分解** → `plans/`
- **设计规范 / 变更提案 / 架构设计 / 能力规范** → `specs/`
- **评审 / 实验结果 / 深度分析 / 流程笔记** → `review/`
